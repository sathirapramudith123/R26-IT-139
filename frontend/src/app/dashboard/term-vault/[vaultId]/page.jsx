...
export default function DetailPage({ params }) {
  ...
  useEffect(() => {
    termVaultApi.getById(params.vaultId)
      .then(setItem).catch(() => setItem(null)).finally(() => setLoading(false));
  }, [params.vaultId]);

  return (
    <div className="page-container">
      <PageHeader title="Term Vault" description="Full details for this record."
        action={<Link href="/dashboard/term-vault"><Button variant="secondary">← Back</Button></Link>} />
      {loading ? <LoadingSpinner /> : !item ?
        <Card><p className="text-slate-500">Record not found. ID: {params.vaultId}</p></Card> :
