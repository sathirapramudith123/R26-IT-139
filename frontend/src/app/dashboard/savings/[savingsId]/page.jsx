...
export default function DetailPage({ params }) {
  ...
  useEffect(() => {
    savingsApi.getById(params.savingsId)
      .then(setItem).catch(() => setItem(null)).finally(() => setLoading(false));
  }, [params.savingsId]);

  return (
    <div className="page-container">
      <PageHeader title="Savings Record" description="Full details for this record."
        action={<Link href="/dashboard/savings"><Button variant="secondary">← Back</Button></Link>} />
      {loading ? <LoadingSpinner /> : !item ?
        <Card><p className="text-slate-500">Record not found. ID: {params.savingsId}</p></Card> :
