...
export default function DetailPage({ params }) {
  ...
  useEffect(() => {
    smartAgentApi.getById(params.agentId)
      .then(setItem).catch(() => setItem(null)).finally(() => setLoading(false));
  }, [params.agentId]);

  return (
    <div className="page-container">
      <PageHeader title="Smart Agent Insight" description="Full details for this record."
        action={<Link href="/dashboard/smart-agent"><Button variant="secondary">← Back</Button></Link>} />
      {loading ? <LoadingSpinner /> : !item ?
        <Card><p className="text-slate-500">Record not found. ID: {params.agentId}</p></Card> :
