...
export const stockStatus = async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from(TABLE).select("*").eq("user_id", req.user.id)
    ...
    const runningOut = data.filter(i => Number(i.quantity) <= Number(i.reorder_level));
    const inStock    = data.filter(i => Number(i.quantity) >  Number(i.reorder_level));

    res.json({
      summary: {
        total_items:   data.length,
        in_stock:      inStock.length,
        running_out:   runningOut.length,
      },
